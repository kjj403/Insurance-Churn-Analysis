...
> **통계/데이터사이언스 최종 과제** > 통신사 구독(Subscription) 데이터를 보험 도메인으로 재해석하여, 고객 세분화(Clustering)부터 이탈 예측 모델링(Predictive Modeling), 그리고 데이터 기반 리텐션 전략 기획까지 수행한 엔드투엔드(End-to-End) 데이터 분석 프로젝트입니다.
...
- **목표:** 1. 행동 패턴 기반의 고객 세분화를 통한 고위험 핵심 타겟 도출
  ...
  3. SHAP(설명 가능한 AI) 기법을 적용한 초개인화 타겟팅 근거 마련
  4. 데이터 기반의 실무적 맞춤형 리텐션(Retention) 전략 도출

<br/>

## 💡 주요 비즈니스 임팩트 (Business Impact)
...
- **핵심 전략:** 1. 이탈 고위험군(군집 2) 타겟 30일 집중 온보딩 프로그램
  ...
  3. 디지털 부가 서비스(온라인 보안 등) 무료 체험을 통한 락인(Lock-in)
...
│   └── modeling_utils.py              # 공통 특성 공학(FE) 및 스플릿 유틸리티
├── README.md
└── requirements.txt